...
class LocMonND:
    ...
    def __init__(self, master):
        ...
        ttk.Label(self.frame_header, text = 'YRC Freight: An Application for Optmizing Dock Layout at YRC Distribution Centers ').grid(row = 0, column = 0)
        ttk.Label(self.frame_header, wraplength = 300, text = ("This is an application that was built using the front end and back end code using the Python. Buit in Spyder, this applicaton can be downloaded on GitHUb. Using the URL (https://github.com/AlesiaStewart/Spring2019_SD.git) "
        ...
        ttk.Label(self.frame_content, text = 'Total time cost equation:').grid(row = 2, column = 10, padx = 4, sticky = 'w')
        ttk.Label(self.frame_content, text = 'Pairwise Layout: ').grid(row = 3, column = 10, padx = 4, sticky ='sw')
        ...
        self.entry_nodoors = ttk.Entry(self.frame_content, width = 24)
        self.entry_timecost = ttk.Entry(self.frame_content, width = 24)
        self.entry_pl= ttk.Entry(self.frame_content, width = 24)

        self.entry_nodoors.grid(row = 3, column = 3,  padx = 5)
        self.entry_timecost.grid(row = 2, column = 11, padx = 5)
        self.entry_pl.grid(row = 3, column = 11, padx = 5)

        ttk.Button(self.frame_content, text = 'Submit', command = self.submit).grid(row = 4, column = 0, padx = 5, pady = 5, sticky = 'e')
        ttk.Button(self.frame_content, text = 'Clear', command = self.clear).grid(row = 4, column = 1, padx = 5, pady = 5, sticky = 'e')
    ...
    def submit(self):
        ...
        print('Total Time Cost Equation with forklift intereferance: {}'.format(self.timecost()))
        self.entry_timecost.insert(0, self.timecost())
        self.entry_pl(0, "N/A")
        print('Pairwise Layout: {}'.format(self.entry_pl.get()))
        self.clear()

    def timecost(self):
        NumOfDoor = LOCATION_DIC[self.entry_nodoors.get()]
        DoorsWide = 11
        DoorWidth = 12
        DoorSpace = 2
    ...
    def clear(self):
        ...
        self.entry_nodoors.delete(0, 'end')
